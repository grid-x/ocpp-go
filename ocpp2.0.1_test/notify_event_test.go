...
func (suite *OcppV2TestSuite) TestNotifyEventDataValidation() {
	...
	var table = []GenericTestEntry{
		...
		{diagnostics.EventData{Timestamp: types.NewDateTime(time.Now()), Trigger: diagnostics.EventTriggerAlerting, EventNotificationType: diagnostics.EventPreconfiguredMonitor, Component: types.Component{Name: "component1"}, Variable: types.Variable{Name: "variable1"}}, true},
